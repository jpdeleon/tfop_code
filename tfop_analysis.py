...
class LPF:
    ...
    def __post_init__(self):
        ...
        self._init_params()
        self._init_ldc()
        self.date4plot = datetime.strptime(self.date, '%y%m%d').strftime('%Y-%b-%d UT')
        if not Path(self.outdir).exists():
            Path(self.outdir).mkdir()
        self.outfile_prefix = f"{self.ticid}{self.alias}_20{self.date}_{self.inst}_{''.join(self.bands)}_{self.model}"
    ...
    def _validate_inputs(self):
        ...
        assert (np.array(self.planet_params['tdur'])<1).all(), "`tdur` must be in days"
    ...
    def plot_raw_data(self, binsize=600/86400, figsize=None, ylims=None):
        ...
        for i,b in enumerate(self.bands):
            ...
            ax.plot(self.times_raw[b]-self.time_offset, self.fluxes_raw[b], 
                    'k.', alpha=0.1, label='raw data')
            tbin, ybin, yebin= lc.binning_equal_interval(self.times[b], self.fluxes[b], 
                                                         self.flux_errs[b], binsize, t0)
            ax.errorbar(tbin, ybin, yerr=yebin, marker='o', c=colors[b], ls='')
            if self.mask_start or self.mask_end:
                mask = self.masks[b]
                ax.plot(self.times_raw[b][mask]-self.time_offset, self.fluxes_raw[b][mask], 
                        'k.', label='masked data')
    ...
    def _init_params(self, return_dict=False):
        ...
        if abs(tc-int_tc)>1:
            ...
            self.norbits = int((np.median(self.times[b]) - tc + 0.3)/self.period[0])
        ...
        if self.use_r1r2:
            ...
            r1_err, r2_err = imp_k_to_r1r2(imp[1], k[1], k_lo=PRIOR_K_MIN, k_up=PRIOR_K_MAX)
            params = {
                'tc': self.tc,
    ...
    def get_chi2_linear_baseline(self, p0):
        ...
        for i,b in enumerate(self.bands):
            ...
            c = np.polyfit(self.covariates[b], self.fluxes[b]-flux_time, self.lm_order)
            linear_model = np.polyval(c, self.covariates[b])
            chi2 += np.sum( (self.fluxes[b] - linear_model + flux_time)**2 / self.flux_errs[b]**2 )
        return chi2
    ...
    def get_chi2_chromatic_transit(self, pv):
        ...
        if self.use_r1r2:
            ...
            if (r1 < 0.) or (r1 > 1.0):
                ...
                return np.inf
            if np.any(r2 < 0.) or np.any(r2 > 1.0):
                if self.DEBUG:
        ...
        if (tc < self.obs_start) or (tc > self.obs_end):
            if self.DEBUG:
                print(f"Error (tc outside data): {self.obs_start:.4f}>{tc:.4f}>{self.obs_start:.4f}")
            return np.inf
        # tc shouldn't be more or less than half a period
        if (tc > 0.) and (abs(tc - self.obs_start) > per/2.):
            if self.DEBUG:
                print(f"Error (tc more than half of period): {tc}-{self.obs_start:.4f} > {per/2:.4f}")
            return  np.inf
        chi2 = 0.
        for i,b in enumerate(self.bands):
            t = self.times[b]
            ...
            e = self.flux_errs[b]
            z = self.covariates[b]
            flux_tr = self.transit_models[b].evaluate_ps(k[i], self.ldc[b], tc, 
                                                         per, a_Rs, inc, e=0, w=0)
    ...
    def optimize_chromatic_transit(self, p0, method='Nelder-Mead'):
        ...
        self.opt_result = minimize(self.get_chi2_chromatic_transit, pv, method=method)
    ...
    def get_upsampled_transit_models(self, pv, npoints=200):
        ...
        for i,b in enumerate(self.bands):
            ...
            ymodel = tmodel.evaluate_ps(k[i], self.ldc[b], tc, 
                                        per, a_Rs, inc, e=0, w=0)
    ...
    def plot_lightcurves(self, pv, binsize=600/86400, ylims=None, figsize=None):
        ...
        for i,b in enumerate(self.bands):
            ...
            tbin, ybin, yebin=\
                lc.binning_equal_interval(t, f/trends[b], e, binsize, t0)
            ...
            _ = self.plot_ing_egr(ax=ax[1], ymin=0.9, ymax=1., color='C0')
            ax[1].axhline(1-depth, color='blue', linestyle='dashed', label='TESS', alpha=0.5)
            if ylims:
                ax[1].set_ylim(*ylims)
    ...
    def sample_mcmc(self, pv=None, nsteps=1_000, nwalkers=None):
        ...
        pos = [params + 1e-5*np.random.randn(self.ndim) for i in range(self.nwalkers)]
                ...
                if self.model=='chromatic':
                    for b in self.bands:
                        ...
                        for i,(r1, r2) in enumerate(zip(r1s,r2s)):
                            imps[i], ks[i] = r1r2_to_imp_k(r1, r2, k_lo=PRIOR_K_MIN, k_up=PRIOR_K_MAX)
                        df[f'k_{b}'] = ks
    ...
    def plot_detrended_data_and_transit(self, pv, title=None, 
                                        xlims=None, ylims=None, 
                                        binsize=600/86400,
                                        msize=5, font_size=20, 
                                        title_height=0.95,
                                        figsize=None):
        """
        pv : list 
        ...
        See also `plot_detrended_data_and_transit()`
        """
        title = f'{self.name}{self.alias} (TIC{self.ticid}{self.alias})' if title is None else title
        figsize = (10,10) if figsize is None else figsize
        ...
        for i,b in enumerate(self.bands):
            ...
            # raw data and binned
            tbin, ybin, yebin=\
                lc.binning_equal_interval(t, detrended_flux, e, binsize, tc)
            ax[i].errorbar((tbin-tc)*24, ybin, yerr=yebin, fmt='ok', markersize=msize)
            xmodel, ymodel = self.get_upsampled_transit_models(pv, npoints=500)[b]
            ax[i].plot((xmodel-tc)*24, ymodel, '-', lw=3, color=colors[b]) 
            ax[i].axhline(1-depth, color='blue', linestyle='dashed', label='TESS', alpha=0.5)
            if self.nband%2==1:
                ax[i].set_ylabel("Relative Flux", fontsize=font_size*0.8)
            ax[i].set_xlabel("Time from transit center (hours)", fontsize=font_size*0.8)
            ...
            ax[i].text(tx, ty, f"{b}-band", color=colors[b], fontsize=font_size*0.8)
        ax[1].set_title(f'{self.date4plot}, {self.inst}', loc='right', fontsize=font_size)
    ...
    def plot_posteriors(self, 
        ...
        for i,b in enumerate(self.bands):
            ...
            ax[0].errorbar(i, k_med, yerr=np.c_[k_err1].T, elinewidth=40, fmt='none', alpha=0.5, zorder=1, color=colors[b])
            ax[0].errorbar(i, k_med, yerr=np.c_[k_err2].T, elinewidth=40, fmt='none', alpha=0.3, zorder=2, color=colors[b])
            ax[0].errorbar(i, k_med, yerr=np.c_[k_err3].T, elinewidth=40, fmt='none', alpha=0.1, zorder=3, color=colors[b])
        ...
        ax[0].text(0., 1.12, title, horizontalalignment='left',\
                 verticalalignment='center', transform=ax[0].transAxes, fontsize=font_size*1.5)
        ax[0].text(0., 1.05, f'{self.model.title()} transit fit,  $\Delta$BIC (non-transit - transit) = {self.bic_delta:.1f}',\
                 horizontalalignment='left', verticalalignment='center', transform=ax[0].transAxes, fontsize=font_size)       
        ...
        # posterior
        n,bins,_ = ax[1].hist(tc, density=True, bins=nbins, zorder=5, label='Posterior')
        ...
        ax[1].set_xlabel(f'Tc (BJD-{self.time_offset:.0f})', labelpad=25, fontsize=font_size*1.5)
        ax[1].legend(loc='best')
        
        imp = df['imp'].values
        _ = ax[2].hist(imp, density=True, bins=nbins)
        ax[2].set_xlabel('Impact parameter', labelpad=25, fontsize=font_size*1.5)
        ax[2].set_title(f'{self.date4plot}, {self.inst}', loc='right', fontsize=font_size*1.5)
        if save:
            outfile = f"{self.outdir}/{self.outfile_prefix}_posteriors"
            outfile += suffix if lpf.mask_start is None else f"_mask{suffix}"
    ...
    def plot_final_fit(self, 
        ...
        fig, ax = plt.subplots(2, self.nband, figsize=figsize, sharey='row', sharex='col')
        ...
        if self.use_r1r2:
            ...
            imp_best, k_best = r1r2_to_imp_k(r1, r2, k_lo=PRIOR_K_MIN, k_up=PRIOR_K_MAX)
        else:
        ...
        for i,b in enumerate(self.bands):
            ...
            flux_tr = self.transit_models[b].evaluate_ps(k_best[i], self.ldc[b], tc_best, 
                                                         per, a_Rs_best, inc_best, e=0, w=0)
            
        
            tbin, ybin, yebin=\
                lc.binning_equal_interval(t, f/trends_best[b], e, binsize, t0)
            # detrended flux
            ...
            ax[0,i].text(tx, ty, f"{b}-band", color=colors[b], fontsize=font_size*0.6)
            ...
            _ = self.plot_ing_egr(ax=ax[1,i], ymin=0.9, ymax=1., color='C0')
            if i==0:
                ...
                target_name = f'{self.name}{self.alias} (TIC{self.ticid}{self.alias})' if title is None else title
                ax[0,i].text(0., 1.14, target_name, horizontalalignment='left',\
                         verticalalignment='center', transform=ax[0,i].transAxes, fontsize=font_size)
                ax[0,i].text(0., 1.05, f'{self.model.title()} transit fit,  $\Delta$BIC (non-transit - transit) = {self.bic_delta:.1f}',\
                         horizontalalignment='left', verticalalignment='center', transform=ax[0,i].transAxes, fontsize=font_size*0.6)
            if (i>0) and (i==self.nband-1):
                ax[0,i].set_title(f'{self.date4plot}, {self.inst}', loc='right', fontsize=font_size*0.8)
            if i>0:
            ...
            ax[1,i].set_xlabel(f'BJD - {self.time_offset:.0f}', labelpad=30, fontsize=font_size)
    ...
    def plot_ing_egr(self, ax, ymin=0.9, ymax=1., color='C0'):
        """
        plot ingress and egress timings over detrended light curve plot
        """
        tdur, tdure = self.tdur
        ing, egr = self.ingress[0], self.egress[0]
        ax.axvspan(ing-tdure/2, ing+tdure/2, alpha=1, ymin=ymin, ymax=ymax, color=color)
        ax.axvspan(ing-3*tdure/2, ing+3*tdure/2, alpha=0.5, ymin=ymin, ymax=ymax, color=color)
        ax.axvspan(egr-tdure/2, egr+tdure/2, alpha=1, ymin=ymin, ymax=ymax, color=color)
        ax.axvspan(egr-3*tdure/2, egr+3*tdure/2, alpha=0.5, ymin=ymin, ymax=ymax, color=color)
        return ax
    ...
    def plot_fov(self, 
        ...
        df = pd.read_csv(ref_obj_file_path, comment="#", names=columns, delim_whitespace=True)
        ...
        fig = plt.figure(figsize=figsize)
        fig.subplots_adjust(top=title_height)  # Adjusted subplots_adjust to give more space to the title
        ...
        if show_target:
            c = SphericalCircle((ra*u.deg, dec*u.deg), rad_marker*u.arcsec, edgecolor='red', facecolor='none',
                                lw=3, zorder=10, transform=ax.get_transform('fk5'))
            ax.add_patch(c)
            # ax.text(ra+dr, dec+dd, 'target', fontsize=20, 
            #         color='red', transform=ax.get_transform('fk5'))
        for i,(r,d) in enumerate(coords):
            j = int(star_ids[i])
            c = SphericalCircle((r*u.deg, d*u.deg), rad_marker*u.arcsec, edgecolor=marker_color, facecolor='none',
        ...
        if show_scale_bar:
            ...
            ax.annotate(text="1\'", xy=(sx-60/pixscale, sy+10), 
                        fontsize=font_size*1.5, color=scale_color, xycoords="data")
    ...
    def plot_fov_zoom(self, 
        ...
        pixscale = header['PIXSCALE']
        band = header['FILTER']
        title = f"{self.name}\n{self.inst} {band[0]}-band (zoomed-in)" if title is None else title
        ...
        if show_scale_bar:
            ...
            ax.annotate(text=f"{bar_arcsec}\"", xy=(sx-bar_arcsec/pixscale, sy+10), 
                        fontsize=font_size*2, color=scale_color, xycoords="data")
        fig.suptitle(title, y=title_height, fontsize=font_size)
        # coord_format="dd:mm:ss"
        # ax.coords[1].set_major_formatter(coord_format)
        # ax.coords[0].set_major_formatter(coord_format)
    ...
    def plot_gaia_sources(self, 
        ...
        for i,(r,d) in enumerate(coords):
            if i==0:
                ...
                lw = 2
                mc = marker_color
            c = SphericalCircle((r*u.deg, d*u.deg), rad_marker*u.arcsec, edgecolor=mc, facecolor='none',
                            lw=lw, transform=ax.get_transform('fk5'))
            ax.add_patch(c)
            ax.text(r+dr, d+dd, 
                    str(i+1), 
                    fontsize=20, 
...
class Star:
    ...
    def __post_init__(self):
        ...
        sources = set([p.get('prov') for i,p in enumerate(self.data_json['stellar_parameters'])])
    ...
    def get_tfop_data(self):
        ...
        self.exofop_url = f"{base_url}/target.php?id={self.name.replace(' ','')}&json"
    ...
    def get_star_params(self):
        ...
        try:
            self.rstar = tuple(map(float, (star_params.get('srad', np.nan), star_params.get('srad_e', np.nan))))
            self.mstar = tuple(map(float, (star_params.get('mass', np.nan), star_params.get('mass_e', np.nan))))
            #stellar density in rho_sun
            self.rhostar = (self.mstar[0]/self.rstar[0]**3, \
                            np.sqrt((1/self.rstar[0]**3)**2 * self.mstar[1]**2 \
            ...
            self.teff = tuple(map(float, (star_params.get('teff', np.nan), star_params.get('teff_e', 500))))
            self.logg = tuple(map(float, (star_params.get('logg', np.nan), star_params.get('logg_e', 0.1))))
    ...
    def get_gaia_sources(self, rad_arcsec=30):
        ...
        if (not hasattr(self, 'gaia_sources')) or (rad_arcsec>30):
            print(f"Querying Gaia sources {rad_arcsec}\" around {self.name}: ({self.ra:.4f}, {self.dec:.4f}) deg.")
            self.gaia_sources = Catalogs.query_region(
                target_coord, radius=rad_arcsec * u.arcsec, catalog="Gaia", version=3
            ).to_pandas()
            self.gaia_sources['distance'] = self.gaia_sources['distance']*u.arcmin.to(u.arcsec)
        self.gaia_sources = self.gaia_sources[self.gaia_sources['distance']<=rad_arcsec]
...
class Planet(Star):
    ...
    def get_planet_params(self):
        ...
        sources = set([p.get('prov') for i,p in enumerate(data_json['planet_parameters'])])
        errmsg = f"{self.source} must be in {sources}"
        assert self.source in sources, errmsg
        ...
        try:
            self.t0 = tuple(map(float, (planet_params.get('epoch', np.nan), planet_params.get('epoch_e', 0.1))))
            self.period = tuple(map(float, (planet_params.get('per', np.nan), planet_params.get('per_e', 0.1))))
            self.tdur = np.array(tuple(map(float, (planet_params.get('tdur', 0), 
                                                   planet_params.get('dur_e', 0)))))/24
            self.rprs = np.sqrt(np.array(tuple(map(float, (planet_params.get('dep_p', 0), 
                                                           planet_params.get('dep_p_e', 0)))))/1e6)
            self.imp = tuple(map(float, (0 if planet_params.get('imp', 0) == '' else planet_params.get('imp', 0), 
                                         0.1 if planet_params.get('imp_e', 0.1) == '' else planet_params.get('imp_e', 0.1))))
            print(f"t0={self.t0} BJD\nP={self.period} d\nRp/Rs={self.rprs}")
            rhostar = self.star_params['rhostar']
            self.a_Rs = ((rhostar[0]/0.01342*self.period[0]**2)**(1/3),
                        1/3*(1/0.01342*self.period[0]**2)**(1/3) * rhostar[0]**(-2/3) * rhostar[1])
        except:
            raise ValueError(f"Check exofop: {self.exofop_url}")
...
def get_toi_ephem(
    target_name: str, idx: int=1, params=["epoch", "per", "dur"]) -> list:
...
def tdur_from_per_imp_aRs_k(per, imp, a_Rs, k):
    ...
    return per/np.pi * np.arcsin(1./a_Rs*np.sqrt((1.+k)**2 - imp**2)/sini)
...
def r1r2_to_imp_k(r1, r2, k_lo=0.01, k_up=0.5):
    ...
    """
    Ar = (k_up-k_lo) / (2.+k_lo+k_up)
    if r1>Ar:
        imp = (1.+k_lo)*(1.+((r1-1.)/(1.-Ar)))
        k = (1.-r2)*k_lo + r2*k_up
    else:
        q1 = r1/Ar
        imp = (1.+k_lo) + np.sqrt(q1)*r2*(k_up-k_lo)
        k = k_up + (k_lo-k_up)*np.sqrt(q1)*(1.-r2)
    return imp, k
...
def imp_k_to_r1r2(imp, k, k_lo=0.01, k_up=0.5):
    ...
    """
    Ar = (k_up - k_lo) / (2. + k_lo + k_up)
    discriminant = 1. + 4. * (1. - Ar) * (imp - (1. + k_lo))
    
    if discriminant >= 0:
        # Case r1 > Ar
        r1 = (1. + k_lo + np.sqrt(discriminant)) / (2. * (1. - Ar))
        r2 = (k - (1. - r1) * k_lo) / (r1 * k_up)
    else:
        # Case r1 <= Ar
        q1 = ((1. + k_lo) - imp) / (k_up - k_lo)
